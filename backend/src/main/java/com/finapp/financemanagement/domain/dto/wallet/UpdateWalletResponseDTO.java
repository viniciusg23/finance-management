...
/**
 * Data Transfer Object (DTO) representing the response for updating a wallet.
 * This DTO contains information about the updated wallet including its ID, profile ID,
 * icon, name, description, color, whether it's a goal wallet, balance, and creation timestamp.
 */
public record UpdateWalletResponseDTO(
    @NotNull UUID id,
    ...
    /**
     * Constructs an UpdateWalletResponseDTO object based on the provided Wallet entity.
     *
     * @param wallet the Wallet entity to map to the DTO
     */
    public UpdateWalletResponseDTO(Wallet wallet) {
        this(
            wallet.getId(),
